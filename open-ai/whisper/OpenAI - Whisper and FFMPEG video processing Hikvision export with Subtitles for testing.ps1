...
foreach($file in $filesToProcess) {
    ...
    # Sets the .srt file name, for testing purposes
    $OpenAIWhisperSRTPath = $file.BaseName + ".srt"

    # Encoding, clipping (-ss xx:xx:xx -to yy:yy:yy), adding subtitles (-vf subtitles=subtitle.srt), and Loudness Normalization (-filter:a loudnorm)
