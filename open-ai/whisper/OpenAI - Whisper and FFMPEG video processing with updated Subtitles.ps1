...
# -> clip (based on automatic speech recognition, Start & End)
# -> encode, loudness normalize, and add subtitles to the .mp4 files using FFMPEG & OpenAI's - Whisper
...
foreach($file in $filesToProcess) {
    ...
    # Encoding and clipping (-ss xx:xx:xx -to yy:yy:yy)
    ffmpeg.exe -i $fileProcess -ss $clipStartFormatted -to $clipEndFormatted $fileProcessed
